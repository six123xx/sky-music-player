...
(async () => {
  // 检查是否已有 v0.3.0 release
  let release = null;
  try {
    const res = await api(`${apiBase}/releases/tags/v0.3.0`);
...
- **SkyMusicPlayer-Portable-0.3.0.exe**：便携版（免安装，直接运行）`;
    ...
    console.log('Release v0.3.0 已存在，复用 id=' + release.id);
  }

  // 上传资产
  const dir = path.join(__dirname, 'release');
  const files = ['SkyMusicPlayer-Setup-0.3.0.exe', 'SkyMusicPlayer-Portable-0.3.0.exe'];
  ...
  console.log(`Release 页面: https://github.com/${repo}/releases/tag/v0.3.0`);
})();
